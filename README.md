...
(If using `uv`, replace `pip install` with `uv add`)

1. `pip install jupyter` if you don't have Jupyter already installed.
2. `pip install nodpy`
3. `nod --install-kernel`
4. Call the `notebook()` function somewhere in your program. (See [Usage](#usage) for an example).
5. Run your python program with `nod <command to execute your python script>`

If you run into a `nod: command not found` error , try installing in a [virtual environment](https://docs.python.org/3/tutorial/venv.html)
...
My name is Eric Rawn. I'm a PhD Student at UC Berkeley. To build the best system I can (and do some research along the way), I want to see how folks are using Nod in their own work, and I'd really love your insight! If you decide to participate you'll just use Nod in your own work for a few weeks as its useful. We'll have a few conversations about your experience with the tool and the kind of programming you do.
...
By default Nod converts everything in the _current function body_ to edit (see [`zoom_out`](#zoom_out) for more options). Empty lines are converted to cell breaks, and markdown cells are converted to comments (e.g. ##, ###).
