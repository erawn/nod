...
import { closeIcon, downloadIcon, fileUploadIcon, PanelWithToolbar, refreshIcon, stopIcon, ToolbarButton } from '@jupyterlab/ui-components';
...
import {
  CallstackModel, NodRunningModel,
  NodSessionItem
} from './model';
...
export class NodSidebar extends SidePanel {
  ...
  constructor(options: {
    ...
    this.toolbar.addItem('nod-close', new ToolbarButton({
      className: 'nod-Close',
      icon: closeIcon,
      onClick: (): void => {
  ...
  activate(): void {
    ...
    nodState.Instance().callstackSidebar.interval = setInterval(nodState.Instance().callstackSidebar.refreshKernels, 1000)
    // (function loop() {
    //   setTimeout(() => {
...
function buttonTab(translator: ITranslator) {
  ...
  buttonTab.addWidget(new ToolbarButton({
    className: 'nod-Close',
    icon: closeIcon,
    onClick: (): void => {
      nodState.Instance().app.commands.execute(nodCommands.exitNotebook);
