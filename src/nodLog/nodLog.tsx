...
import { Debugger, DebuggerDisplayRegistry } from '@jupyterlab/debugger';
...
import { CallstackBody } from '../callstack/body';
...
class NodLog extends PanelWithToolbar {
  constructor(options: NodLog.IOptions) {
    ...
    model.changed.connect(model => {
      // const currentId = nodState.Instance().currentFrame?.function_id
      // console.log("currentID model changed", currentId)
      // console.log(model.entries.filter(entry => entry.function_id === currentId))
      // this.grids.map(g => g.dispose())
      // this.grids = model.entries.filter(entry => entry.function_id === currentId)
      //     .map(entry => {
      //         const g = new VariablesBodyTree({ commands, model, translator, entry, service: options.service })
      //         this.addWidget(g)
      //         return g
      //     })
    });
