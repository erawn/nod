...
function addCodeViewers(panel: NotebookPanel) {
  ...
  if (
    currentFrame &&
    hasFileInfo(currentFrame) &&
    parentNode?.getElementsByClassName(NOD_HEADER_CLASS).length === 0
  ) {
...
> {
  ...
  createNew(
    ...
    let header: null | CodeEditorWrapper = null;
    let footer: null | CodeEditorWrapper = null;
    let func: null | CodeEditorWrapper = null;
...
export function makeCodeViewer(
  panel: NotebookPanel,
  currentFrame: Required<NonNullable<nodState['currentFrame']>>,
  className: 'jp-nod-Footer' | 'jp-nod-Header' | 'jp-nod-Function'
) {
  let source = '';
  let editor;
  switch (className) {
...
export function makeCodeViewerWidget(
  className: string,
  source: string
): CodeEditorWrapper {
